...
def get_llm_analysis_summary(analysis_path='meeting_analysis.txt'):
    """Parse LLM analysis file for key metrics."""
    ...
    # Extract key metrics
    key_points = len(re.findall(r'^\d+\.\s+', content, re.MULTILINE))
    decisions = content.count('## Decisions Made')
    action_items = content.count('**Task**:')
    follow_ups = content.count('## Follow-up Items')
    
    # Extract effectiveness score
    effectiveness_match = re.search(r'Effectiveness Score\*\*:\s*(\d+)', content)
    effectiveness = int(effectiveness_match.group(1)) if effectiveness_match else None
...
def print_summary_table(audio_duration=None, proc_time=None, ram=None, tstats=None, sstats=None, llm_analysis=None, wer=None, der=None):
    print("\nSummary Table\n" + "-"*40)
    ...
    if llm_analysis and llm_analysis.get('available'):
        print(f"{'--- LLM Analysis ---':<30} {'---'}")
        print(f"{'Key Discussion Points':<30} {llm_analysis.get('key_points', 'N/A')}")
        print(f"{'Decisions Identified':<30} {llm_analysis.get('decisions', 'N/A')}")
        print(f"{'Action Items Extracted':<30} {llm_analysis.get('action_items', 'N/A')}")
        print(f"{'Follow-up Items':<30} {llm_analysis.get('follow_ups', 'N/A')}")
        if llm_analysis.get('effectiveness'):
            print(f"{'Meeting Effectiveness':<30} {llm_analysis['effectiveness']}/10")
...
if __name__ == "__main__":
    ...
    llm_analysis = get_llm_analysis_summary()
    ...
    print_summary_table(audio_duration, proc_time, ram, tstats, sstats, llm_analysis, wer, der)
